...
//verifie que le fichier possede un entete compatible
	...
	//tab contient les x(t)
	
	//fenetrage de Hamming
	int T=nbech/header.frequency; //temps total de l'enregistrement
	for (int i=0; i<=nbech; i++) {
		tab[i][0]=tab[i][0]*(0.54-0.46*cos(2*pi*i/(header.frequency*T)));
		 //expression du fenetrage de hamming
		//ajouter -lm lors du compilage, pour le cos
		//tab[i][1]=tab[i][1]*(0.54-0.46*cos(2*pi*i/(header.frequency)*T));
		}
	//tab contient x(t)*h(t) avec h le fenêtrage de Hamming
		
		
	for (int i=nbech+1; i<taille; i++) {
	tab[i][0]=0;
	}
	...
	//int gsl_fft_complex_radix2_forward (gsl_complex_packed_array dat, const size_t stride,const size_t taille);
	double *tab_concat=malloc(taille*2*sizeof(double));
	for (int i=0; i<taille-1; i++) {
		tab_concat[2*i]=tab[i][0];
		tab_concat[2*i+1]=tab[i][1];
		printf("%f ", tab_concat[2*i]);
		/*if (i%2==0) {
			tab_concat[i]=tab[i][0];
		} else{
			tab_concat[i]=tab[i][1];
		}*/
	}
	...
	for(i=0;i<taille;i++) {
		free(tab[i]);
		tab[i] = NULL ;
		} //on libèrera à la fin

//on essaie d'enregistrer les échantillons de la fft dans notre tableau

/*
while( fread(&value,(header.bits_per_sample)/8,1,dat) ) {
	tab[i][0]=value;
	i++;
	}
*/
//déduisons en la fréquence fondamentale
//on trace tableau2 de 0 à Fe/2 (donné dans structure)
...
//gcc -Wall -Werror lecture.c  $(gsl-config --cflags --libs) -lm
